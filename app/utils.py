...
def get_text_chunks(text: str, model_type: str = "Google AI") -> list[str]:
    """
    Splits a given text into smaller, manageable chunks suitable for embeddings.

    Args:
        text (str): The input text to be chunked.
        model_type (str): Specifies the model type to adjust chunking parameters.
...
def get_vector_store(text_chunks: list[str], api_key: str) -> FAISS:
    ...
    Args:
        ...
        api_key (str): Your Google AI API key for embeddings.
...
def load_or_create_vector_store(file_path: str, api_key: str, index_path: str) -> FAISS:
    """
    Loads an existing FAISS vector store from disk or creates a new one if it doesn't exist.

    Args:
        file_path (str): Path to the source PDF file.
        api_key (str): Your Google AI API key.
        index_path (str): Directory where the FAISS index is/will be stored.
    ...
    # Check for FAISS index existence (both .faiss and .pkl files are needed)
    ...
    if not os.path.exists(faiss_file) or not os.path.exists(pkl_file):
        ...
        if not pdf_text:
            raise ValueError(f"Could not extract text from PDF at {file_path}. Cannot create vector store.")
        chunks = get_text_chunks(pdf_text, MODEL_NAME)
        vector_store = get_vector_store(chunks, api_key)
        print("Vector store created and saved.")
    else:
        print("Loading existing FAISS vector store...")
        # allow_dangerous_deserialization=True is needed if you trust the source of the index
        vector_store = FAISS.load_local(index_path, embeddings, allow_dangerous_deserialization=True)
        print("Vector store loaded.")
    return vector_store

# In-memory store for chat history sessions (this will hold ChatMessageHistory objects)
# This is a simple in-memory store; for production, consider Redis or a database.
_session_store = {}
def get_session_history(session_id: str) -> BaseChatMessageHistory:
    """
    Returns a ChatMessageHistory object for a given session ID.
    This is used by RunnableWithMessageHistory to store raw messages.
    """
